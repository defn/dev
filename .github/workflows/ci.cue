...
jobs: {
	build_nix_amd: {
		steps: #DockerSteps + [{
			...
			run: """
				make docker-nix \\
					cache=--cache-from=ghcr.io/${GITHUB_REPOSITORY}-cache:main-amd-nix \\
	...
	build_amd: {
		...
		steps: #DockerSteps + [{
			...
			run: """
				make docker-dev \\
					cache="--cache-from=ghcr.io/${GITHUB_REPOSITORY}-cache:main-amd-nix --cache-from=ghcr.io/${GITHUB_REPOSITORY}-cache:main-amd" \\
	...
	build_k3d_amd: {
		...
		steps: #DockerSteps + [{
			...
			run: """
				make docker-k3d \\
					cache="--cache-from=ghcr.io/${GITHUB_REPOSITORY}-cache:main-amd-nix --cache-from=ghcr.io/${GITHUB_REPOSITORY}-cache:main-amd-k3d" \\
