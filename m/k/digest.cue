...
cached_image: {
	...
	output: {
		...
		if cache[c.image] != _|_ {
			image: "coder-amanibhavam-district.tail3884f.ts.net:5000/\(cache[c.image])"
		}
		if cache[c.image] == _|_ {
			image: "\(c.image)"
		}
			...
			if r.spec != _|_ {spec: {
				...
				if r.spec.image != _|_ {image: {
					if cache[r.spec.image] != _|_ {
						...
						"coder-amanibhavam-district.tail3884f.ts.net:5000/\(strings.Join(list.Slice(_parts, 1, len(_parts)), "/"))"
					}
					if cache[r.spec.image] == _|_ {
