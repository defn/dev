	...
	if [[ $OSTYPE == "linux"* ]]; then
		if [[ $BREW != "" ]]; then
			...
			*) ARCH=$(uname -m) ;;
			...
			set -x
			curl -fsSL https://github.com/defn/dev/m/tilt/releases/download/v$VERSION/tilt.$VERSION.linux.$ARCH.tar.gz | tar -xzv tilt
			copy_binary
		fi
		...
		if [[ $BREW != "" ]]; then
			...
			ARCH=$(uname -m)
			set -x
			curl -fsSL https://github.com/defn/dev/m/tilt/releases/download/v$VERSION/tilt.$VERSION.mac.$ARCH.tar.gz | tar -xzv tilt
			copy_binary
		fi
	else
		set +x
		...
		echo "https://github.com/defn/dev/m/tilt/issues/new"
		echo "Thank you!"
		exit 1
	fi
