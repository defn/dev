...
def docker_build(
    ...
    When using Docker Compose, Tilt expects the image build to be either managed by your Docker Compose file (via the `build <https://docs.docker.com/compose/compose-file/compose-file-v3/#build>`_ key) OR by Tilt's :meth:`docker_build`, but not both. (Follow this `GitHub issue <https://github.com/defn/dev/m/tilt/issues/5196>`_ to be notified of changes to this expectation.)
...
def docker_compose(
    configPaths: Union[str, Blob, List[Union[str, Blob]]],
    env_file: str = None,
    project_name: str = "",
...
TRIGGER_MODE_AUTO = type(
    "_sentinel", (TriggerMode,), {"__repr__": lambda self: "TRIGGER_MODE_AUTO"}
)()
...
def k8s_resource(
    ...
    new_name: str = "",
    port_forwards: Union[
        str, int, PortForward, List[Union[str, int, PortForward]]
    ] = [],
...
def load(path: str, *args):
    ...
    Example ::
      ...
      load('ext://hello_world', 'hi') # Resolves to https://github.com/defn/dev/m/tilt-extensions/blob/master/hello_world/Tiltfile
...
def k8s_kind(
    ...
    api_version: str = None,
    *,
    image_json_path: Union[str, List[str]] = [],
    image_object_json_path: Dict = None,
    pod_readiness: str = ""
):
    ...
    Custom Resource
    <https://github.com/defn/dev/m/tilt/blob/master/integration/crd/Tiltfile#L8>`_.
...
def read_yaml_stream(
    path: str, default: List[StructuredDataType] = None
...
def default_registry(
    host: str, host_from_cluster: str = None, single_name: str = ""
) -> None:
...
def ci_settings(
    k8s_grace_period: str = "", timeout: str = "30m", readiness_timeout: str = "5m"
) -> None:
...
def http_get_action(
    port: int, host: str = "localhost", scheme: str = "http", path: str = ""
) -> HTTPGetAction:
