...
kustomize: "traefik": #KustomizeHelm & {
	...
	psm: "service-traefik": {
		...
		metadata: {
			...
			namespace: "traefik"
			annotations: {
				"external-dns.alpha.kubernetes.io/internal-hostname": "*.district.\(cluster.handle).\(cluster.domain_zone), *.school.\(cluster.handle).\(cluster.domain_zone), *.coder.district.\(cluster.handle).\(cluster.domain_zone), *.coder.school.\(cluster.handle).\(cluster.domain_zone)"
