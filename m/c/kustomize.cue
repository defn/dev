	...
	resource: "awsnodetemplate-default": {
		...
		spec: {
			...
			userData: """
				...
				(cd m/pkg/k3sup && nix develop --command k3sup join --user ubuntu --server-host coder-amanibhavam-dev --server-user ubuntu --k3s-extra-args "--node-ip ${container_ip} --node-external-ip ${ts_ip}")
				EOF
