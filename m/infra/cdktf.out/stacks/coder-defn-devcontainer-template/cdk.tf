...
resource "null_resource" "main" {
  ...
  provisioner "local-exec" {
    when = create
    command = "( (echo pkill -9 -f coder.agent '||' true; echo pkill -9 -f code-server '||' true; echo cd; echo export STARSHIP_NO=1 CODER_AGENT_TOKEN=${coder_agent.main.token}; echo source .bash_profile; echo ${base64encode(coder_agent.main.init_script)} | base64 -d) | ssh ${data.coder_parameter.remote.value} /home/ubuntu/m/node_modules/.bin/devcontainer exec --workspace-folder /home/ubuntu/m bash -x - >>/tmp/startup3.log 2>&1 &) &"
  }
}
