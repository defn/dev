...
resource "aws_organizations_account" "klamath" {
...
resource "aws_ssoadmin_account_assignment" "klamath_admin_sso_account_assignment" {
  ...
  principal_id       = aws_identitystore_group.administrators_sso_group.group_id
  principal_type     = "GROUP"
  target_id          = aws_organizations_account.klamath.id
  target_type        = "AWS_ACCOUNT"
}
resource "aws_organizations_account" "tahoe" {
  email = "tahoe@defn.us"
  name  = "tahoe"
  tags = {
    ManagedBy = "Terraform"
...
resource "aws_ssoadmin_account_assignment" "tahoe_admin_sso_account_assignment" {
  ...
  principal_id       = aws_identitystore_group.administrators_sso_group.group_id
  principal_type     = "GROUP"
  target_id          = aws_organizations_account.tahoe.id
  target_type        = "AWS_ACCOUNT"
}
moved {
  from = aws_organizations_account.jianghu-org
  to   = aws_organizations_account.jianghu
}
moved {
  from = aws_organizations_account.jianghu-log
  to   = aws_organizations_account.klamath
}
moved {
  from = aws_organizations_account.jianghu-net
  to   = aws_organizations_account.tahoe
}
