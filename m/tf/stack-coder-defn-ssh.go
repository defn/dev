...
func CoderDefnSshStack(scope constructs.Construct, site *infra.AwsProps, name string) cdktf.TerraformStack {
	...
	deploy.AddOverride(infra.Js("provisioner"), []map[string]map[string]string{
		{"local-exec": {
			"when": "create",
			"command": fmt.Sprintf("(echo cd; echo exec env CODER_AGENT_URL=%s CODER_AGENT_TOKEN=%s CODER_NAME=%s CODER_HOMEDIR=%s CODER_INIT_SCRIPT_BASE64=%s TS_AUTH_KEY=$(~/bin/ts-auth-key 2>&1) %s) | %s bash -x -",
