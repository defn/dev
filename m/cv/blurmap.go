...
// generateJavaScriptIndex creates a JavaScript file with an indexed blurhash object instead of an array
func generateJavaScriptIndex(imageInfos []ImageInfo, outputFile string) error {
	...
	out.WriteString("// Generated blurhash index object\n")
	out.WriteString("// This file is auto-generated by blurmap.go with RGB color values\n")
	out.WriteString("window.blurhashIndex = {\n")

	// Process each image to include in the index
	for _, info := range imageInfos {
		...
		// Default blurhash value
		blurmap := info.Blurmap
		
		// If we don't have a blurhash, generate one
		if blurmap == "" {
			var genErr error
			blurmap, _, _, genErr = generateBlurmap(imagePath)
			if genErr != nil {
				// Skip this entry if we can't generate a blurhash
				continue
			}
		}

		// Add entry to index - use both the full filename and basename as keys
		basename := filepath.Base(info.Filename)
		
		// Add the full filename entry
		fmt.Fprintf(out, "  \"%s\": { filename: \"%s\", width: %d, height: %d, blurhash: \"%s\" },\n",
			info.Filename, info.Filename, width, height, blurmap)
		
		// Also add basename entry if different from full filename
		if basename != info.Filename {
			fmt.Fprintf(out, "  \"%s\": { filename: \"%s\", width: %d, height: %d, blurhash: \"%s\" },\n",
				basename, info.Filename, width, height, blurmap)
		}
	}
