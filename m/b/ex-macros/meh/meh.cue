...
#Model: {
  ...
  loads: [...{
    bzl:     string             // file to load
    symbols: [...string]        // loaded symbols
  }]
...
// 1) raw genrule with multiple outs
t_raw: {
  kind: "genrule"
  name: "raw_configs"
...
// 2) normalized genrules (one per normalize step), plus a filegroup
t_norm: [
...
// 4) macro invocations (archive_directory) and info (archive_info)
t_bundles: [
  for b in m.bundles {
    {
...
_loads: [ for l in m.loads { (renderLoad & {#in: l}).out } ]
_loadsJoined: strings.Join(_loads, "\n")

_targets: [
  for t in targets {
    if t.kind == "genrule" {
      (renderGenrule & {#in: t}).out
    }
    if t.kind == "filegroup" {
      (renderFilegroup & {#in: t}).out
    }
    if t.kind == "macro.archive_directory" {
      (renderArchiveDir & {#in: t}).out
    }
    if t.kind == "macro.archive_info" {
      (renderArchiveInfo & {#in: t}).out
    }
  }
]
_targetsJoined: strings.Join(_targets, "\n")
