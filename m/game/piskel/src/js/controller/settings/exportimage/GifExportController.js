(function () {
  ...
  ns.GifExportController.prototype.onUploadButtonClick_ = function (evt) {
    ...
    this.renderAsImageDataAnimatedGIF(
      zoom,
      fps,
      this.uploadImageData_.bind(this),
    );
  };
  ...
  ns.GifExportController.prototype.onImageUploadCompleted_ = function (
    imageUrl,
  ) {
  ...
  ns.GifExportController.prototype.onImageUploadFailed_ = function (
    event,
    xhr,
  ) {
    if (xhr.status === 500) {
  ...
  ns.GifExportController.prototype.updateStatus_ = function (imageUrl, error) {
    if (imageUrl) {
      var linkTpl =
        '<a class="highlight" href="{{link}}" target="_blank">{{shortLink}}</a>';
      var linkHtml = pskl.utils.Template.replace(linkTpl, {
        link: imageUrl,
        shortLink: this.shorten_(imageUrl, URL_MAX_LENGTH, "..."),
      });
      this.uploadStatusContainerEl.innerHTML =
        "Your image is now available at : " + linkHtml;
    } else {
      // FIXME : Should display error message instead
    }
  };

  ns.GifExportController.prototype.shorten_ = function (
    url,
    maxLength,
    suffix,
  ) {
