...
resource "digitalocean_volume_attachment" "dev" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "set -x; cd && git fetch && git reset --hard origin/main && unset DOCKER_HOST && env DEFN_DEV_HOST=${each.value.host} DEFN_DEV_IP=${each.value.ip} make provision-digital-ocean"
    ]
  }
